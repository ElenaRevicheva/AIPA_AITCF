...
## 🆕 June 12 2026 — public proof layer actualized + blog deploy semantics fixed + client-facing test shipped same-day

### 1. Blog static pages now deploy themselves (cto-aipa `1cc388a`, live on Oracle)
**Incident:** `/blog/0-to-1-transferable-skills` 404'd on IPFS ("no link named … under bafybei…"). Root cause: `src/blog-static-pages.ts` committed every per-article static page with `[skip ci]` — so a NEW article's page sat in git invisible on 4everland until an unrelated commit happened to trigger a build. Silent SEO damage on every publish.
**Fix:** `[skip ci]` is now appended only when **updating** an existing page (GitHub Contents API `sha` present). A new page (no sha) commits normally → exactly one deploy per new article. Deploy-storm protection for bulk regenerates preserved. Deployed: pull + build + `pm2 restart cto-aipa`, verified `online` via `pm2 jlist | jq`.
**Rule earned: "A new public artifact must trigger its own deploy."** If a pipeline creates something users can link to, the pipeline owns making it reachable — not the next unrelated commit.

### 2. Public proof layer actualized to June 2026 state (aideazz repo, commits `83fd5df` → `d742a6c`)
- **SOP pages (EN + ES)** — `public/sop-ai-ops.html` / `-es.html`: dates to June 2026; Hashnode pipeline → bilingual daily blog (aideazz.xyz/blog + dev.to crosspost, mutex/dedup/notify); stack rows gained Grok (xAI) tier-3 failover, Whisper retry+fallback chain, Bright Data lead-gen, FAQPage JSON-LD; radar chips for live-web layer + 3-tier failover; **NEW second postmortem: "The engagement loop that never ran"** (the verify-from-logs story, now public interview ammo).
- **Favicon fixed at the root** — `/favicon.ico` was the stale ugly mark and pages pointed at the 854 KB PNG (muddy at 16px). Regenerated multi-size `favicon.ico` + crisp `favicon-32.png` + `apple-touch-icon.png` from the real AIdeazz triangle (sharp + png-to-ico, `--no-save`); wired `?v=3` icon set into index.html, both SOPs, pitch.html.
- **Portfolio (`/portfolio`)** — architecture diagram label fixes (CMO AIPA was mislabeled "Job Hunting" → "Marketing & CRM"; CTO → "Control Tower"; VibeJob → "Job Hunting"; EspaLuz "Life Assistant" → **"Spanish Tutor · Relocation"** — the old label read as spiritual-mentor vague). NEW capability chips under the diagram (HubSpot multi-agent pipeline · Bright Data research agent · bilingual GEO/SEO/AEO blog · voice briefing · 3-tier failover) + **three proof buttons: Ops Runbook · Blog · Podcast** (blog/podcast were buried on the card's flip side). **Honest count enforced in 8 places EN+ES: "10-agent ecosystem — 9 live 24/7"** (was "10 Live Agents", contradicting the SOP). Positioning line upgraded from bare "Applied AI Engineer" to resume-aligned: **"Applied AI Engineer & AI Systems Builder | Agents · AI Marketing · CRM & Revenue Automation | Founder @ AIdeazz"** (+ JSON-LD jobTitle). Timeline 13 → 15 months (March 2025 — June 2026).

### 3. Tech Specialist hiring test built + twice-tested live in under 2 hours (June 12 morning)
Client-onboarding system for a hiring pipeline: **one bootstrap Apps Script `setup()`** builds Drive folder tree, formatted tracker + dashboard, intake form, onFormSubmit trigger (row + client folder + email in ~5s), and the SOP Google Doc — infrastructure-as-code instead of n8n (deliberate, defensible choice). First live test caught a real bug (`+phone` parsed as a Sheets formula → `#ERROR!`); fixed with a text guard + re-tested clean. **Technique earned:** OAuth consent popups that open in unreachable browser windows can be re-homed — hook `window.open` in page context to capture the consent URL, `location.assign()` it in the controlled tab, click through there. Artifacts in `Desktop\TechSpecialistTest\` + memory `project_tech_specialist_test.md`.
