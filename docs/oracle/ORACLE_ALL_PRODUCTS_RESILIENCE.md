...
> **AELA vs AILA — cleared up Aug 16 2026.** Oracle carried an idle `/home/ubuntu/aela/` (lowercase) that was easy to mistake for AILA. It was a **different repo** — [`ElenaRevicheva/AELA`](https://github.com/ElenaRevicheva/AELA), branch `docs` — 254 MB, 10,079 files, last touched 20 March, **no process, no systemd unit, no PM2 entry**. It also held a `.env` with `ANTHROPIC_API_KEY` + `OPENAI_API_KEY`, and read the EspaLuz OpenAI key.
>
> **Removed from Oracle Aug 16 2026** after verifying it was safe: Oracle's HEAD `5ce3f8e` was *behind* `origin/docs` `c98f3e7`, **0 unpushed commits**, and nothing unique outside `.claude/` (tool settings) and the `.env`. Backup kept at `~/backups/aela-oracle-copy.*.tgz` (69 MB). **The code lives on in GitHub — nothing was lost.**
>
> Removing it also cut one copy of two API keys off the VM. If AELA is ever needed again, clone it fresh from GitHub rather than expecting it on disk.
