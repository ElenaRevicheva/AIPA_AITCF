...
**✅ Resolved (June 19 2026):** Sprinter Lambda `GITHUB_TOKEN` was expired (every `/repos/...` query 401'd, repo section degraded). Refreshed the Lambda env var (`aws lambda update-function-configuration`, us-east-1, function `sprint-briefing-agent`) with the valid Oracle `.env` `GITHUB_TOKEN` (user ElenaRevicheva, `repo` scope, verified 200). Native invoke confirmed: dedup OK, `/sprint-knowledge` proxy OK (4369 chars), Trello OK, **no more GitHub 401**. To refresh again: `GH=<valid-pat> py scripts/update-sprinter-token.py` (merges env, preserves all 18 vars). Note: the Lambda's narrative gen is Claude→Groq only (no Grok/Gemini tier) — on rare days when BOTH are exhausted (Anthropic credits + Groq daily cap) it can't write the script; Groq's daily cap resets, so next 8AM recovers. Durability TODO: add a Grok/Gemini fallback tier to the Lambda bundle like the server has.
