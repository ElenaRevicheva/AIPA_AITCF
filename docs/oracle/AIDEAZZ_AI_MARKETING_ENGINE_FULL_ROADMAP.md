...
| **GSC / ops follow-up** | Manual | **Re-submit** `https://aideazz.xyz/sitemap.xml` after deploy. **Request removal** for **`/.gitignore`** if still indexed. **Verify** sitemap URL returns **200** (if **500**, likely CDN/IPFS — redeploy and retry). |
