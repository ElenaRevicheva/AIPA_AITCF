...
async function startCTOAIPA() {
  ...
  /** Display title from a film filename: strip stamp, `__` → ` // `, dashes/underscores → spaces,
   *  then Title-Case plain lowercase words while PRESERVING already-cased/dotted words
   *  (e.g. `Atuona.xyz__Paradise.js-<stamp>.mp4` → "Atuona.xyz // Paradise.js"). */
  const filmTitle = (name: string): string =>
    name.replace(/\.mp4$/i, '').replace(/-\d{4}-\d{2}-\d{2}T.*$/, '').replace(/__/g, ' // ').replace(/[-_]/g, ' ').trim()
      .split(' ')
      .map(w => (/[A-Z]/.test(w) || w.includes('.') || !/^[a-z]/.test(w)) ? w : w.charAt(0).toUpperCase() + w.slice(1))
      .join(' ');
  ...
  app.get('/films', (req: Request, res: Response) => {
    ...
    const cards = films.map(f => {
      // (title casing handled server-side by filmTitle — pages must NOT apply CSS capitalize,
      // Chrome uppercases after dots and would mangle e.g. "Atuona.xyz" into "Atuona.Xyz")
      const url = `${base}/films/${encodeURIComponent(f.name)}${keyQ}`;
      const when = new Date(f.mtimeMs).toISOString().slice(0, 16).replace('T', ' ');
      const title = filmTitle(f.name);
...
<style>body{margin:0;background:#0b0b0d;color:#eee;font-family:-apple-system,Segoe UI,Roboto,sans-serif}header{padding:24px 16px;text-align:center;border-bottom:1px solid #1c1c1f}h1{margin:0;font-weight:300;letter-spacing:.35em}.sub{color:#888;font-size:13px;margin-top:6px;letter-spacing:.04em}.wrap{max-width:780px;margin:0 auto;padding:16px}.film{margin:0 0 34px}.film h2{font-family:Georgia,'Times New Roman',serif;font-weight:400;font-size:23px;letter-spacing:.02em;margin:0 0 4px}.meta{color:#777;font-size:12px;margin-bottom:8px}video{width:100%;border-radius:8px;background:#000}a{color:#9ad;text-decoration:none;font-size:13px}.empty{color:#888;text-align:center;padding:48px}footer{text-align:center;color:#666;font-size:11px;letter-spacing:.06em;line-height:2;padding:34px 16px 46px;border-top:1px solid #1c1c1f;margin-top:26px}footer a{color:#8a8a90}</style></head>
