...
export async function runSerpProspects(opts: { dryRun?: boolean } = {}): Promise<{
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    await pushToCRM({
      ...
      sourceUrl: c.link,
      amount:   offer.amount,
      draftSubject: `${offer.label} for you — quick walkthrough?`,
      draftBody: [
        `Hi there,`,
        ``,
        `Saw this signal: ${v!.label}`,
        ``,
        `I build ${offer.label} (${renderOfferEstimate(offer)} range) — WhatsApp/Telegram bots, LLM wiring, automation, GEO/AEO, AI product video.`,
        `Happy to show a 15-min demo if useful.`,
        ``,
