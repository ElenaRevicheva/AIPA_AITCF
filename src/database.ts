...
export interface AtlasPerformanceMetrics {
  ...
  /** GA4 nightly sync (scripts/sync-atlas-ga4.mjs) — distinct from generic sessions to avoid double-count. */
  ga4_sessions?: number;
  ga4_key_events?: number;
  /** WhatsApp redirect clicks (future /go/wa adapter). */
  wa_clicks?: number;
}
...
function sumMetrics(rows: Array<{ metrics: AtlasPerformanceMetrics }>): AtlasPerformanceMetrics & {
  ...
  for (const r of rows) {
    ...
    t.ga4_sessions += Number(m.ga4_sessions) || 0;
    t.ga4_key_events += Number(m.ga4_key_events) || 0;
    t.wa_clicks += Number(m.wa_clicks) || 0;
  }
